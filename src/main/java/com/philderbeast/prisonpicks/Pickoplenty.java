            ...
            while (x < bX + radius)
            {
                ...
                while (y < bY + radius)
                {
                    ...
                    while (z < bZ + radius)
                    {
                        ...
                        if (distance < (double)(radius * radius) && Util.canBuild(player, check) && (check.getBlock().getType() != Material.BEDROCK) && (check.getBlock().getType() != Material.AIR) && ( ! check.getBlock().hasMetadata("blockBreaker")) && Priority.getPriority((Material)check.getBlock().getType()).level >= level)
                        {
