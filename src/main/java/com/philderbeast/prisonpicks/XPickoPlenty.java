            ...
            while (x < bX + radius)
            {
                ...
                while (y < bY + radius)
                {
                    ...
                    while (z < bZ + radius)
                    {
                        ...
                        if (distance < (double)(radius * radius) && Util.canBuild(player, block) && (block.getBlock().getType() != Material.BEDROCK) && (block.getBlock().getType() != Material.AIR))
                        {
                            locations.add(block); 
                            if ((level < Priority.getPriority(block.getWorld().getBlockAt(block).getType()).level) &&  ! block.getBlock().hasMetadata("blockBreaker"))
                            {
